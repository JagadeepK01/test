...
class NeuDataMongoDBOperator(object):
    """Represents a document client.

    Provides base functions for Mongo DB
    """
    conn_type = 'mongo'
    ...
    def GetMongoDataJSON(self, connection, database, collection,batchSize):
        """Gets query output in JSON
        :param str query:
            SQL query string
        :param str cursor
            Cursor object
        ...
        #cursor = collection.find({}, {'_id': 0 }).batch_size(batchSize)
        cursor = collection.find_raw_batches(batch_size=batchSize)
        return cursor
    ...
    def insert_many(self, database, collection, docs, **kwargs):
        """
